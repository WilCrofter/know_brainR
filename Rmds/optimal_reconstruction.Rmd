...
recover_checkerboard <- function(board, diameter, noise_to_signal){
  ...
  noise_power <- noise_to_signal*mean(Mod(scalp_fft)^2)
...
In that note no attempt at optimal recovery was made, but recovery by linear regression was mentioned. Assuming patterns of depolarization are spatially uncorrelated, (a pessimistic assumption,) recovery by linear regression is straightforward. For this method, simulations pictured below indicate that recovery of details at $n \gt 4$ mm resolution require noise-to-signal amplitude ratios at the scalp at least 2 orders of magnitude smaller than that of the eigenvalue at spatial frequency $\frac{1}{n}$ cycles/mm.

Recovery of details at resolutions smaller than 4 mm seem to require noise-to-signal amplitude ratios of 4 or 5 orders of magnitude smaller than that of the associated eigenvalue.
...
plot_recovery <- function(diameter, cell_size, resolution=cell_size, noise_to_signal=eigenv(resolution)/100){
  ...
  display_centered_image(board, diameter=diameter, main= paste("Checkerboard depolarization pattern\nwith cell size ", cell_size, "(eigenvalue ", signif(eigenv(resolution), 4),")"))
  display_centered_image(recovery, diameter=diameter, main=paste("Recovered pattern, noise-to-signal\namplitude ratio", signif(noise_to_signal, 4)))
  par(mfrow=c(1,1))
}
```

To illustrate the first point consider a checkerboard pattern 50 mm in diameter with cell 10 mm on a side. Cells are alternately polarized (white,) and depolarized (red.) There are no emissions from the area colored green. (See the [previous note](discrete_methods.html) for details.) With noise-to-signal two orders of magnitude below the eigenvalue at spatial frequency 1/10 cycles/mm, the checkerboard pattern is discernible.

```{r echo=FALSE, fig.align='center', fig.show='hold'}
plot_recovery(50, 10)
```

If the noise-to-signal ratio is greater than two orders of magnitude above the appropriate eigenvalue, the checkerboard is not discernible.

```{r echo=FALSE, fig.align='center', fig.show='hold'}
plot_recovery(50, 10, noise_to_signal=eigenv(10)/10)
```

This heuristic holds down to 4 mm resolution;

```{r echo=FALSE, fig.align='center', fig.show='hold'}
plot_recovery(50, 4)
```

But does not hold below 4 mm resolution.
