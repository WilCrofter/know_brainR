...
recover_checkerboard <- function(board, diameter, noise_to_signal){
  power <- checkerboard_power(diameter)
  noise_power <- power*noise_to_signal^2
  # form fft of scalp pattern and add noise
  scalp_fft <- fwd_xfer_fct*fft(board)/512 + 
    matrix(rnorm(512*512, 0,sqrt(noise_power)), 512, 512)
  # form denominator of optimal inverse filter
  dinv <- fwd_xfer_fct + noise_power/Mod(scalp_fft)^2/Conj(fwd_xfer_fct)
...
In that note no attempt at optimal recovery was made, but recovery by linear regression was mentioned. Assuming patterns of depolarization are spatially uncorrelated, (a pessimistic assumption,) recovery by linear regression is straightforward. For this method, simulations pictured below indicate that recovery of details at $n \gt 4$ mm resolution require noise-to-signal amplitude ratios at least 3 orders of magnitude smaller than that of the eigenvalue at spatial frequency $\frac{1}{n}$ cycles/mm. Recovery of details at resolutions smaller than 4 mm seem to require noise-to-signal amplitude ratios of 4 or 5 orders of magnitude smaller than that of the associated eigenvalue.

```{r echo=FALSE, fig.align='center', fig.show='hold'}
plot_recovery <- function(diameter, cell_size, resolution=cell_size, noise_to_signal=eigenv(resolution)/1000){
  par(mfrow=c(1,2))
